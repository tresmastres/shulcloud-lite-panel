...
export default function Families(){
  ...
  return (
    <div className="p-6 space-y-6">
      <SectionTitle title="Familias" subtitle="Gestiona miembros y facturación por familia" />
      ...
      <div className="flex gap-2">
        ...
        <button
          className="btn"
          onClick={() => alert('Aquí iría el formulario para crear una nueva familia')}
        >
          Nueva familia
        </button>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
          ...
          <div key={f.name} className="bg-white rounded-xl shadow p-4 space-y-2">
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                onClick={() => alert('Aquí iría el formulario para añadir miembro a ' + f.name)}
              >
                Añadir miembro
              </button>
            </div>
            <table className="w-full text-sm">
              ...
              <tbody>
                    ...
                    <tr key={m.id} className="border-t">
                      ...
                      <td className="flex gap-2 py-2">
                        ...
                        <button
                          ...
                          onClick={() => alert('Formulario para añadir factura al miembro ' + code)}
                        >
                          Factura
                        </button>
                        <button
                          className="btn btn-xs"
                          onClick={() => alert('Formulario para registrar cobro del miembro ' + code)}
                        >
                          Cobro
                        </button>
                        <button
                          className="btn btn-xs"
                          onClick={() => alert('Formulario para editar al miembro ' + code)}
                        >
                          Editar
                        </button>
                      </td>
                ...
                {f.members.length===0 && (
                  <tr>
                    <td colSpan="3" className="py-2 text-slate-400 text-center">
                      Sin miembros
                    </td>
                  </tr>
                )}
        ...
        {filtered.length===0 && (
          <p className="text-slate-400">Sin familias</p>
        )}
      </div>
    </div>
  )
